...
| После SPEC + ADR-001 | "Требования верны? Стек устраивает?" | Изменение FR, AC, приоритетов, выбор технологий |
...
6. docs/decisions/ADR-001-tech-stack.md — фиксация технологического стека (ОБЯЗАТЕЛЬНО)
7. ⏸️ СТОП: "Требования верны? Стек устраивает?"
```

### Этап 3: Дополнительные ADR (при необходимости)
```
8. docs/decisions/ADR-002-*.md и далее — если есть другие архитектурные решения:
...
| [ADR.md](templates/ADR.md.template) | Архитектурное решение | ADR-001-tech-stack обязателен (Этап 2), остальные по необходимости (Этап 3) |
